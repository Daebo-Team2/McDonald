...
public class StockOrderDAO {
	...
	public List selectAllStockOrder( ) { //본사 : 발주 요청 조회 1) select * from stockorderlist
		...
		try {
			...
			System.out.println("selectAllStockOrder !!!");

			String sql = "SELECT * FROM STOCKORDER WHERE STATUS = 0 ORDER BY NO DESC ";
//			String sql = "SELECT SO.NO, SO.STORENO, SO.TIME, SO.STATUS, SL.STOCKORDERNO, SL.FOODNO, SL.QUANTITY FROM STOCKORDER SO INNER JOIN STOCKORDERLIST SL ON SO.NO = SL.STOCKORDERNO AND SO.STATUS = 0 ";
	  ...
	  System.out.println("selectAllStockOrderList !!!");
	  ...
	  while( rs.next() ) { StockOrderListVO vo = new StockOrderListVO();
	  vo.setFoodno(rs.getInt("foodno"));
	  vo.setQuantity(rs.getInt("quantity"));
	  
	  list.add(vo); } System.out.println(list);
